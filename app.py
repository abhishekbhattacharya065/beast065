...
class Tinder:
    ...
    def select_dp(self,flag=1):
        if flag==0:
            self.filename=filedialog.askopenfilename(initialdir="/images", title="Select from device")
            self.dp_filename.config(text=self.filename)
            dpInput=self.filename.split("/")[-1]
            destination="C:\\Users\\ACER\\source\\repos\\Tinder\\Tinder\\images\\" + dpInput
    ...
    def reg_handler(self):
        ...
        flag=self.db.register(self._nameInput.get(),self._emailInput.get(),self._passwordInput.get(),self._ageInput.get(),self._genderInput.get(),self._cityInput.get(),actual_filename)

        if flag==1:
            # File upload
            
            destination="C:\\Users\\ACER\\source\\repos\\Tinder\\Tinder\\images\\" + actual_filename
            shutil.copyfile(self.filename,destination)
    ...
    def edit_profile(self):
        ...
        self.filename="default/"+str(data[0][7])
        self.dp=Button(self._root, text="Update Profile Picture", command=lambda: self.select_dp(flag=0))
        self.dp.pack(pady=(5,5))
        
        self.dp_filename=Label(self._root,text=self.filename)
        self.dp_filename.pack(pady=(5,5))
    ...
    def update_info(self):
        ...
        dpInput=self.filename.split("/")[-1]
        flag=self.db.update_profile(self._emailInput.get(),self._passwordInput.get(),self._ageInput.get(),self._cityInput.get(),dpInput,self.user_id)
